...
\texttt{ucs()}, \texttt{weighted\_astar()} and \texttt{greedy\_best\_first()} do no post-processing
at all. Comparing their reported times therefore compares different regions, and the asymmetry
falls entirely on the arm we were measuring. It is worth a median of 26.0\,ms on these paths
(range 7.0--55.2\,ms), which on the $\mathrm{A}^*$ arm is not a rounding error. We time that
post-processing on the identical path and subtract it, so Table~\ref{tab:weff} reports search-only
time for every arm.
...
\paragraph{(ii) The advantage is real and amortisable, and the break-even depends entirely on the
baseline.} The setup constant depends only on the graph, the traveller context, and the cost preset
--- not on the query --- so hoisting it out of the query loop is legitimate for any workload that
issues repeated queries in one context. Against uniform-cost search, $\mathrm{A}^*$+admissible saves
136.6\,ms of search per query, so the 1599\,ms setup pays for itself after $11.7$ queries; weighted
$\mathrm{A}^*$+fast breaks even after $7.6$. Against bidirectional uniform-cost search the picture
is very different: it searches in 155.4\,ms, barely slower than $\mathrm{A}^*$+admissible's
146.1\,ms, so that pairing needs $172$ queries to amortise, while weighted $\mathrm{A}^*$+fast needs
$19.3$. A previous draft claimed $\mathrm{A}^*$+admissible \emph{never} breaks even against
bidirectional search; on the corrected timings that is wrong --- it does, just two orders of
magnitude later than against the weaker baseline.
...
$11.7$ queries to repay. And for single-query exact routing the mechanism that actually wins is
bidirectional expansion --- structure, not information --- which no amount of query volume is
needed to justify.
...
\item \textbf{Search.} The heuristic's information is a global constant obtained by one $O(|E|)$
  ...
  $6.18\times$; past 12 queries against uniform-cost search, or 172 against bidirectional search,
  it wins. Node counting reports the $n \to \infty$ limit unconditionally, and never names either
  the assumption or the baseline it is relative to.
...
tests. The headline effect is large ($6.18\times$) and the invariance result is exact, but the
timing means are medians over five repetitions with a run-to-run spread of 6--21\%, and we report
no confidence intervals on them. \S\ref{sec:pso} uses 30 paired seeds, adequate for the reported

...
\textbf{(ii)~Constraint satisfaction:} ordering and propagation cut node counts largely as folklore
predicts ($-3$ to $-5$ nodes, $p \le 10^{-4}$; minimum-remaining-values alone cuts none,
$p = 0.27$) while running $12$--$14\times$ slower; past the point where search stops completing,
the node metric inverts and measures throughput rather than pruning.
...
algorithms (uniform-cost, Dijkstra \citep{dijkstra1959note}, bidirectional uniform-cost,
$\mathrm{A}^*$, weighted $\mathrm{A}^*$, greedy best-first) run over three named heuristics, giving
twelve configurations on each of ten randomly drawn landmark pairs.
...
dispersion --- specifically the Brown--Forsythe variant \citep{brown1974robust}, which centres on
the median rather than the mean --- on 30 paired seeds for Problem~3 and 5 for Problem~4. We report
$p$-values and make no claim of ``better'' without a test. We report disconfirmations of our own
expectations in the same register as confirmations; three of the four sections below contain at
least one.
...
paper reported the two as separate configurations with different timings, which was pure timing
noise on identical code presented as a comparison; in the run reported here the same two entry
points come out at 282.7\,ms and 279.0\,ms. We merge them. The study describes ``six search
algorithms''; it has five distinct ones behind six entry points. This is the same failure as the
heuristic collapse, on the other axis of the grid, and our own audit rule (\S\ref{sec:synthesis})
did not initially catch it because we applied the rule only to the heuristics.
...
(\texttt{repro/\allowbreak path\_cost\_probe.py}) --- negligible beside $\mathrm{A}^*$'s
...
rates. We planned with assumed arrival rates scaled by
$\lambda \in \{0,\, 0.1,\, 0.25,\, 0.5,\, 0.75,\, 1,\, 1.25,\, 1.5,\, 2\}$ against

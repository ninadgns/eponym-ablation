...
\item A released, deterministic testbed: four problems, four reference implementations, and
  scripts that regenerate every table in this paper. Problems~3 and~4 carry 23 and 28 tests
  respectively, all passing with no skips; Problems~1 and~2 carry 7 and 18, and Problem~2's are
  not runnable from its shipped lockfile (\S\ref{sec:threats}).
\end{enumerate}
...
label overwritten (\texttt{algorithms.py:118--131}; its docstring says ``same implementation as UCS
...
$O(\text{path length})$ and costs 0.73\,ms and 0.58\,ms on average here --- negligible beside
$\mathrm{A}^*$'s $O(\text{path length}^2)$ term, and we leave it in place rather than claim a
precision we do not have. $\mathrm{A}^*$'s post-processing is the one that matters: a median of
...
standard deviations of $0.3$ to $2.0$ minutes, almost every one of these differences sits inside
...
\paragraph{Test coverage is uneven across the four problems.} Problems~3 and~4 carry 23 and 28
tests and are the two whose algorithmic claims we lean on hardest; both suites pass with no skips,
and both encode the load-bearing invariants directly (the budget-exactness assertion in
\S\ref{sec:pso}, and a test that greps the $Q$-learner's own source to confirm it never reads the
true model in \S\ref{sec:rl}). Problems~1 and~2 are weaker: 7 and 18 tests respectively, and
Problem~2's suite is not runnable from its shipped lockfile at all, which does not list
\texttt{pytest}. We verified Problem~2's solvers by re-running them and inspecting outputs rather
than through its tests, and readers should weight \S\ref{sec:csp} accordingly.
...
milliseconds are hardware-dependent.
